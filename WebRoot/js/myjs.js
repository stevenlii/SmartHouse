...
function detailhomes(){
	if(selectedTr!=null)
	 {
		var id =selectedTr.cells[0].innerHTML;
...
function deletehomes(){
	...
	{   
		...
		if(confirm("【该房产有业主】\n确定删除?"))
		{
			...
			var number =selectedTr.cells[1].innerHTML;
//			var number =selectedTr.cells[1].childNodes[0].value;
...
function zhengshiruzhi()
{
	var s="";
	 if(selectedTr!=null)
	 {	 
		 var id =selectedTr.cells[7].innerHTML;
			id = id.substring(id.indexOf('>')+1)
		 s=id;
...
function yinchanghomes()
{
	...
	if(selectedTr.cells[6].childNodes[0].value=="已入住")
	{
		...
		var homesid =selectedTr.cells[0].innerHTML;
		homesid = homesid.substring(homesid.indexOf('>')+1)
		document.getElementById("yezhu_homesid").value=homesid;
		var homesnumber =selectedTr.cells[1].innerHTML;
		homesnumber = homesnumber.substring(homesnumber.indexOf('>')+1)
		document.getElementById("yezhu_homesnumber").value=homesnumber;
		}
	}
	else alert("请选择一个房产");
}
...
//			var id =selectedTr.cells[0].childNodes[0].value;
			id = id.substring(id.indexOf('>')+1)
			var number =selectedTr.cells[2].innerHTML;
			number = number.substring(number.indexOf('>')+1)
